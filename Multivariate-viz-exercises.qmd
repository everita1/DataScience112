...
The range of the distribution is about 800 to 1100. The most typical outcomes are 900 and about 1025. The shape is slightly left skewed. The shape is bi-modal with two distinct peaks.
...
As the spending goes up, the sat score goes down. This means that there is a negative relationship between spending and SAT scores. I find this surprising. I would think that the more the district invests in students and teachers, the higher their scores would be.
...
```{r}
ggplot(education, aes(y = sat, x = salary, color = expend)) + geom_point() +
  geom_smooth(method = "lm")
...
As salary and expend go up, the sat scores of the students go down.
...
The states with a higher percentage of students taking the test have a lower average score. This may be because in the other states, only students who have studied more take the test, skewing the overall average score. In states where the test is mandatory, less students study and therefore the average score goes down. 
...
All the different colored groups divided by fracCat show a positive association between sat score and expenditure. Therefore, the real difference lies in the percentage of students taking the test, rather than expenditure. 
...
The data at first seemed to indicate that there was a negative association between expenditure and sat scores. However, another unobserved variable was actually the cause behind this association. When it was added to the plot, the real relationships between the variables emerged. 
