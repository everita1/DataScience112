...
The graph is bimodal, meaning there were probably two different typical months. One was around 50, while the other was much higher, around 400. The range is from o to 450 therms.
...
There was an increase whenever the years would change, which I assume is in the winter months.
...
After renovation, the therms would increase on average.
...
The cofounding variable is hdd. The temperature is colder after the renovations, so the price is higher to heat the houses.

```{r}
ggplot(energy, aes(x = renovated, y = hdd)) +
  geom_boxplot() +
  facet_wrap(~address)
...
Starbucks has many locations in North America, Europe, and South East Asia. They have little to no locations in South America, Africa, Russia, Australia, North Korea, etc.
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_rent)) +
  ...
  theme(legend.position = "right") + 
  coord_equal() +
  scale_fill_gradientn(name = "median rent", colors = c("blue", "lightblue", "darkblue"))
```
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_age)) +
  ...
  theme(legend.position = "right") + 
  coord_equal() +
  scale_fill_gradientn(name = "median age", colors = c("blue", "lightblue", "darkblue"))
```
