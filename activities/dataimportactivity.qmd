...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Desktop/compproject/112/IMDB/data/imdb_5000_messy.csv")
```
...
```{r}
imdb_temp <- read_csv("../../112/IMDB/data/imdb_5000_messy.csv")
```
...
```{r}
 imdb_messy |>
  ...
  # convert characters to factors in order to summarize
   summary()
```
...
the mean, min, median, 1st and 3rd quartiles, etc. 
...
The categorical variables in particular need categories to be consistent and clean. 
...
imdb_messy |> 
  count(color)
```
Color has many different categories. Most that can be combined, like COLOR and color and Color. As well as B&W and Black and White. Also, there are 19 NAs. 
...
We can share this code with others. 
