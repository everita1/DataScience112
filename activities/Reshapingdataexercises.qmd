...
The people and their sleep habits.
...
`ggplot(sleep_wide), aes(y = reaction time, x = subject, color = day ))`

The days and reaction time for the data sets need to be in one column, that way we can fit them under a single variable. That way, each row represents a subject/day format to graph.
...
reaction time generally worsens (gets longer) with lack of sleep, though some subjects are more affected than others. For instance, 308 has a longer reaction time than 351.
...
```{r}
ggplot(billboard, aes(y = wk1, x = wk2))+
  geom_point() +
  geom_abline(intercept = 0, slope = 1)
```

Generally, most songs improved their rankings in the second week compared to the first week.
...
This activity was medium difficulty for me. I had trouble with some of the wrangling, especially at the end. However, the pivot functions were pretty clear to me, which I think was one of the main goals of the activity, so I am satisfied overall.
