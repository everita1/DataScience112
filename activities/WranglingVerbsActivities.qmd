...
```
It filters out all the states except for Delaware and Hawaii. 
...
The code created a new column that was the difference in percentage between democrats and republicans in 2020.
...
The code used the existing percentage data along with the data on total votes in 2020 to calculate the total votes of republicans in 2020. 
...
# What did this code do?
 elections_small |> 
   ...
   head()
```
Using the data on the percentage of republican and democratic votes in 2020, this code determined whether the republicans either won or lost in the county. 
...
The chunk will filter out all the states except for Wisconsin, and arrange the counties in descending order from highest to lowest votes. 
...
*you get rid of certain rows before arranging the new data.* 
...
The chunk will produce produce the counties in delaware and show which countries republicans one or lost in. 
```{r}
 elections_small |> 
...
filter Minnesota, and select county name and dem vote percentage. You can also arrange from highest to lowest, but it isn't really necessary. 
...
- Minnesota and Wisconsin counties, filter

- Mutate, change in dem vote % in 2020 and 2016

- arrange, counties from highest to lowest
...
It gives a value for the median republican vote percentage in 2020. 
...
Defines and names the value as "median_repub"
...
Creates the value for median republican vote percentage, as well as total votes. 
...
# (What if we didn't use group_by?)
 elections_small |> 
   ...
   summarize(median_repub = median(repub_pct_20), total_votes = sum(total_votes_20)) 
```
Gives the median republican votes and total votes by state. 
...
*we need group_by, summarize, and arrange*
...
We need mutate, group_by, and summarize
...
We need mutate, group_by, summarize, and filter
...
goalsper_game <- world_cup |> 
  mutate(goalsper_game = goals_scored / games)

ggplot(goalsper_game, aes(x = year, y = goalsper_game)) +
  geom_point() +
  geom_line()
...
This activity was relatively straightforward. I still need to get used picking the right verb for the right purpose. For example, sometimes I still get confused on whether to use select, filter, or group by. 

I hope that in future activities and readings, I will be able to more easily combine the different verbs to wrangle the data more specifically. 
